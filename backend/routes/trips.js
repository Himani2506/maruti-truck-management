...
const {
  runAlerts,
  updateCustomerRate,
  updateCustomerRatePerPiece,
} = require("../db/alertEngine");
...
function calcTotals(fields) {
  ...
  const total_cash_expense =
    total_expenses -
    ((parseFloat(diesel_cost) || 0) + (parseFloat(maintenance_rokhar) || 0));
...
router.get("/", async (req, res) => {
  try {
    ...
    let idx = 1;
    if (truck_id) {
      conditions.push(`t.truck_id             = $${idx++}`);
    ...
    if (source_id) {
      conditions.push(`t.source_id            = $${idx++}`);
    ...
    if (from_date) {
      conditions.push(`t.start_date          >= $${idx++}`);
    ...
    if (backload_supplier_id) {
      conditions.push(`t.backload_supplier_id = $${idx++}`);
      values.push(parseInt(backload_supplier_id, 10));
    }
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const distance_km =
      ms_val && me_val && me_val > ms_val
        ? parseFloat((me_val - ms_val).toFixed(2))
        : null;

    let truck_avg = null;

    if (truck_id) {
      const tr = await pool.query("SELECT avg_kmpl FROM trucks WHERE id=$1", [
        truck_id,
      ]);
      if (tr.rows.length) truck_avg = parseFloat(tr.rows[0].avg_kmpl) || null;
    }
    const diesel_needed =
      distance_km && truck_avg
        ? parseFloat((distance_km / truck_avg).toFixed(2))
        : null;

    const diesel_cost = diesel_used
      ? parseFloat((parseFloat(diesel_used) * DIESEL_PRICE).toFixed(2))
      : null;
    ...
    const totalFreight =
      (parseFloat(freight_amount) || 0) +
      (parseFloat(backload_freight_amount) || 0);

    const surplus = totalFreight
      ? parseFloat((totalFreight - total_expenses).toFixed(2))
      : null;

    const status = end_date ? "completed" : "open";

    const loading_unloading =
      (parseFloat(loading_amount) || 0) + (parseFloat(unloading_amount) || 0) ||
      null;
    ...
    if (Array.isArray(customer_ids) && customer_ids.length > 1) {
      const extras = customer_ids.filter(
        (id) => parseInt(id) !== parseInt(customer_id),
      );
      for (const cid of extras) {
        const extraPieces = req.body.customer_pieces?.[cid] || null;
        const extraFreight = req.body.customer_freight?.[cid] || null;
        await pool.query(
          `INSERT INTO trip_customers (trip_id, customer_id, pieces, freight_amount)
     VALUES ($1, $2, $3, $4) ON CONFLICT (trip_id, customer_id)
     DO UPDATE SET pieces = $3, freight_amount = $4`,
          [tripId, cid, extraPieces, extraFreight],
        );
      }
    }
    runAlerts(pool, tripId);
    updateCustomerRate(pool, tripId);
    updateCustomerRatePerPiece(pool, tripId);
...
router.patch("/:id", async (req, res) => {
  try {
    const ex = await pool.query("SELECT * FROM trips WHERE id=$1", [
      req.params.id,
    ]);
    if (!ex.rows.length)
      return res.status(404).json({ error: "Trip not found" });
    ...
    const diesel_cost = u.diesel_used
      ? parseFloat((parseFloat(u.diesel_used) * DIESEL_PRICE).toFixed(2))
      : null;

    let num_days = trip.num_days,
      fooding = trip.fooding,
      trip_bhatta = trip.trip_bhatta;
    ...
    const totalFreight =
      (parseFloat(u.freight_amount) || 0) +
      (parseFloat(u.backload_freight_amount) || 0);
    ...
    const loading_unloading =
      (parseFloat(u.loading_amount) || 0) +
        (parseFloat(u.unloading_amount) || 0) || null;
    ...
    if (Array.isArray(u.customer_ids)) {
      await pool.query(
        `DELETE FROM trip_customers WHERE trip_id = $1 AND customer_id != $2`,
        [req.params.id, u.customer_id],
      );
      const extras = u.customer_ids.filter(
        (id) => parseInt(id) !== parseInt(u.customer_id),
      );
      for (const cid of extras) {
        await pool.query(
    ...
    runAlerts(pool, parseInt(req.params.id));
    updateCustomerRate(pool, parseInt(req.params.id));
    updateCustomerRatePerPiece(pool, parseInt(req.params.id));
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get("/export/excel", async (req, res) => {
  try {
    ...
    if (backload_supplier_id) {
      conditions.push(`t.backload_supplier_id = $${idx++}`);
      values.push(backload_supplier_id);
    }
    const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
    ...
    const result = await pool.query(
      ...
      LEFT JOIN backloads b  ON t.backload_supplier_id = b.id
      ${where}
      ORDER BY tr.truck_number, t.start_date`,
      values,
    );
    ...
    Object.keys(truckGroups)
      ...
      .forEach((truckNumber) => {
        ...
        truckGroups[truckNumber].forEach((row, i) => {
          ...
          if (row.status === "open")
            r.getCell("status").font = { color: { argb: "FFCC0000" } };
          if (row.status === "verified")
            r.getCell("status").font = { color: { argb: "FF006600" } };
          if (row.surplus < 0)
            r.getCell("surplus").font = { color: { argb: "FFCC0000" } };
        });

        const totalsRow = ws.addRow({
          id: "TOTAL",
          total_expenses: truckGroups[truckNumber].reduce(
            (s, r) => s + (parseFloat(r.total_expenses) || 0),
            0,
          ),
          total_cash_expense: truckGroups[truckNumber].reduce(
            (s, r) => s + (parseFloat(r.total_cash_expense) || 0),
            0,
          ),
          freight_amount: truckGroups[truckNumber].reduce(
            (s, r) => s + (parseFloat(r.freight_amount) || 0),
            0,
          ),
          backload_freight_amount: truckGroups[truckNumber].reduce(
            (s, r) => s + (parseFloat(r.backload_freight_amount) || 0),
            0,
          ),
          surplus: truckGroups[truckNumber].reduce(
            (s, r) => s + (parseFloat(r.surplus) || 0),
            0,
          ),
          diesel_cost: truckGroups[truckNumber].reduce(
            (s, r) => s + (parseFloat(r.diesel_cost) || 0),
            0,
          ),
        });
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=maruti_trips.xlsx",
    );
